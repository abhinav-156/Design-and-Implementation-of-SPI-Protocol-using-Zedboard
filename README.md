...
This project validates the fundamental working principle of SPI — a fast, full-duplex, synchronous communication protocol — using a hardware-based setup.
...
|:--|:--|:--|
| MOSI | Master → Slave | Transfers data from the Master |
| MISO | Slave → Master | Sends data from the Slave |
| SCLK | Master → Slave | Clock signal generated by the Master |
| SS | Master → Slave | Slave Select (active low) signal |
...
3. **Data Reception:**  
   ...
   After receiving 8 bits, the full byte is displayed on the Slave’s LEDs.

4. **Synchronization Logic:**  
   Both Master and Slave are governed by FSMs (Finite State Machines) ensuring deterministic communication:
   - **Master FSM:** IDLE → LOAD → TRANSFER → DONE  
   - **Slave FSM:** IDLE → RECEIVE → DISPLAY  
...
## 🧾 Conclusion
The **SPI protocol** was successfully designed, simulated, and implemented using **two ZedBoards** in a **Master–Slave configuration**.  
...
## 📸 Project Images
- https://github.com/abhinav-156/Design-and-Implementation-of-SPI-Protocol-using-Zedboard/blob/main/SPI%20Master%20Schematic%20diagram.jpg 
- https://github.com/abhinav-156/Design-and-Implementation-of-SPI-Protocol-using-Zedboard/blob/main/SPI%20Slave%20Schematic%20diagram.jpg
...
5. Observe 8-bit LED pattern on the Slave board corresponding to the Master’s switch input.
